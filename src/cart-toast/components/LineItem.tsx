...
export const LineItem: FC<LineItemProps> = ({ index, item, quantity, lineItemPrice, lineItemDiscountedPrice, updateCart }) => {
  return (
    <li key={index} className={`cart-toast__item`}>
      ...
      <div className={`cart-toast__item-quantity`}>
        <QuantityInput item={item} quantity={quantity} lineItemPrice={lineItemPrice} updateCart={updateCart} lineItemDiscountedPrice={lineItemDiscountedPrice} />
      </div>
    </li>
  );
};
